    ...
    return (
        <Accordion type="single" collapsible className="w-full mx-auto mb-8 px-5">
            <AccordionItem value="challenges" className="text-center mb-4">
                ...
                <AccordionContent className="grid grid-cols-3 max-sm:grid-cols-1 gap-4 place-items-stretch px-4 mt-3 text-lg">
                    {challenges.length > 0 ? (challenges.map((challenge, index) => (
                        <div
                            key={index}
                            className="group"
                        >
                            <Card key={index} className={`flex flex-col h-full p-4 ${challenge.progress >= challenge.target_value ? 'bg-green-100 dark:bg-green-900 border-green-500 ' : 'bg-yellow-100 dark:bg-yellow-900 border-yellow-500'} border-b-5 shadow-md hover:shadow-lg transition-shadow duration-300 ease-in-out pb-0 group-hover:-translate-y-2 transition-transform duration-200`}>
                                <CardHeader>
